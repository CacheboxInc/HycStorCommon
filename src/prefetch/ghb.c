...
void ghb_init(ghb_t *ghb, ghb_params_t *gparams) {
  ...
  ghb->priv = dc_alloc_and_init(ghb->params.n_lookback);
...
int query_history(ghb_t *ghb, int idx, uint64_t *prefetch_lbas, int prefetch_depth, bool *is_strided) {
  ...
  */
  ...
  return dc_get_requests(dc_struct, last_lba, prefetch_lbas, prefetch_depth, is_strided);
}
...
int ghb_update_and_query(ghb_t *ghb, uint8_t ctx, uint64_t lba, uint64_t *prefetch_lbas, 
  ...
  if (update_index_and_history(ghb, idx, ctx, lba))
	return query_history(ghb, idx, prefetch_lbas, prefetch_depth, is_strided);
